...
- **S7** [Feature-005, 2026-06-25; annotated F016, 2026-07-07]: S2's `__return_true`-ban does NOT apply to RFC-mandated body-authenticated endpoints. The OAuth token endpoint at `/wp-json/acrossai-mcp/v1/token` was the canonical exemption — RFC 6749 §2.3.1 specifies authentication via `client_id` + `client_secret` POST body, not session or header. Exemption requires: (a) the RFC section cited in the class docblock, (b) a documented in-callback validation chain that preserves S2's intent, (c) exactly one `__return_true` match across `includes/OAuth/` (T079 polish grep enforces). **Post-F016 (2026-07-07): NO active consumers — `includes/OAuth/TokenController.php` and the entire `includes/OAuth/` directory were retired. The exemption REMAINS VALID for any future RFC-mandated body-auth endpoint but has zero live consumers. Any new `__return_true` route citing S7 as precedent MUST independently re-justify against RFC 6749 §2.3.1 (or the relevant RFC section) rather than referencing this precedent as still-active-in-use.** See historical: `specs/005-oauth-connectors/contracts/token-endpoint.md`.
