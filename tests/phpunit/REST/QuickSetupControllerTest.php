<?php
/**
 * F069 T041 — QuickSetupController REST test.
 *
...
use AcrossAI_MCP_Manager\Includes\REST\QuickSetupController;
use WP_REST_Request;
use WP_UnitTestCase;

// phpcs:disable Squiz.Commenting.FunctionComment.Missing -- descriptive test names.

final class QuickSetupControllerTest extends WP_UnitTestCase {
	...
	public function setUp(): void {
		...
		QuickSetupController::instance()->register_routes();
		...
		delete_transient( 'acrossai_mcp_manager_quick_setup_state_' . $this->admin_id );
	}
	...
	public function test_permission_check_returns_true_for_admin(): void {
		...
		$this->assertTrue( QuickSetupController::instance()->permission_check() );
	}

	public function test_permission_check_returns_false_for_subscriber(): void {
		wp_set_current_user( $this->subscriber_id );
		$this->assertFalse( QuickSetupController::instance()->permission_check() );
	}
	...
	public function test_post_step_rejects_invalid_step_value_400(): void {
		...
		$this->assertSame( 'acrossai_mcp_quick_setup_invalid_step', $response->get_data()['code'] );
	}
	...
	public function test_post_step_1_select_existing_server_persists_to_scratchpad(): void {
		...
		$state_req = new WP_REST_Request( 'GET', '/acrossai-mcp-manager/v1/quick-setup/state' );
	...
	public function test_post_complete_returns_204_and_clears_scratchpad(): void {
		...
		$req = new WP_REST_Request( 'POST', '/acrossai-mcp-manager/v1/quick-setup/complete' );
		$response = rest_get_server()->dispatch( $req );

		$this->assertSame( 204, $response->get_status() );
		$this->assertFalse(
			get_transient( 'acrossai_mcp_manager_quick_setup_state_' . $this->admin_id ),
	...
	public function test_post_complete_idempotent_second_call_still_204(): void {
		...
		$req2 = new WP_REST_Request( 'POST', '/acrossai-mcp-manager/v1/quick-setup/complete' );
