...
use AcrossAI_MCP_Manager\Admin\Partials\QuickSetup\AdminBarEntry;
use WP_Admin_Bar;
use WP_UnitTestCase;
...
final class AdminBarEntryTest extends WP_UnitTestCase {
	...
	public function test_node_title_contains_wizard_label(): void {
		...
		$node = $bar->get_node( self::NODE_ID );
		$this->assertStringContainsString( 'Quick Setup for MCP', (string) $node->title );
	...
	public function test_node_href_targets_wizard_step_one(): void {
		...
		$this->assertStringContainsString( 'quick-setup=1', (string) $node->href );
