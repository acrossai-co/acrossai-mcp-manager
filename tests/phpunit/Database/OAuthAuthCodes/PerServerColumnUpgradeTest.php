<?php
/**
 * Feature 032 (US3, T015) — regression coverage for the D28 3-part contract on
 * the `server_id` column added to `wp_acrossai_mcp_oauth_auth_codes` at
 * Table version 1.0.0 → 1.0.1.
 *
 * Mirrors T013 / T014 shape. No new indexes on this table.
 *
 * @package AcrossAI_MCP_Manager\Tests\Database\OAuthAuthCodes
 */

declare( strict_types = 1 );

namespace AcrossAI_MCP_Manager\Tests\PHPUnit\Database\OAuthAuthCodes;
...
class PerServerColumnUpgradeTest extends WP_UnitTestCase {
	...
	public function test_server_id_column_exists_after_upgrade_with_not_null(): void {
		...
		$this->assertCount( 1, $rows, 'F032 must ship the server_id column on oauth_auth_codes.' );
	...
	public function test_insert_with_server_id_succeeds_and_without_fails(): void {
		...
		$ok = $wpdb->insert(
			$table,
			array(
				'code_hash'             => hash( 'sha256', 't015-with-' . random_bytes( 8 ) ),
				'client_id'             => 'server-1-t015-with',
				'server_id'             => 1,
				'user_id'               => 1,
		...
		$fail = $wpdb->insert(
			$table,
			array(
				'code_hash'             => hash( 'sha256', 't015-without-' . random_bytes( 8 ) ),
				'client_id'             => 'server-1-t015-without',
				'user_id'               => 1,
				'redirect_uri'          => 'https://client.example.com/callback',
	...
	public function test_upgrade_to_1_0_1_recreates_dropped_column(): void {
		...
		$this->assertSame( '1.0.1', (string) get_option( 'acrossai_mcp_oauth_auth_codes_db_version' ) );
	}

	public function test_no_new_indexes_added_on_auth_codes_table(): void {
		// AuthCodes upgrade adds NO new indexes (unlike Tokens which adds
		// KEY(server_id, client_id)). Verify no `server_id_*` index exists.
		global $wpdb;
		$table = $wpdb->prefix . 'acrossai_mcp_oauth_auth_codes';

		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared
		$idx = $wpdb->get_results( "SHOW INDEX FROM `{$table}` WHERE Key_name LIKE 'server_id%'" );
		$this->assertCount( 0, $idx, 'No server_id-prefixed index expected on oauth_auth_codes.' );
	}
}
