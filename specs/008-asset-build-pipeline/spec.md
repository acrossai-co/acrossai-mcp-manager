...
- **FR-017**: When the OAuth consent surface is active, `public/Main.php::enqueue_styles()` MUST enqueue the `acrossai-mcp-frontend-oauth` handle (full prefixed form — see §Key Entities) sourced from `build/css/frontend-oauth.css` with version and dependencies read from `build/css/frontend-oauth.asset.php`.
