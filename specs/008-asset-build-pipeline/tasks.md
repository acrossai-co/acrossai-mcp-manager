...
- [x] T013 [P] [US3] Create `tests/phpunit/Public/MainEnqueueTest.php`. Cases: (a) no consent-surface context (no `acrossai_mcp_auth` query var, OAuth predicate returns false) → `enqueue_styles()` and `enqueue_scripts()` register ZERO handles; (b) OAuth predicate returns true → `acrossai-mcp-frontend-oauth` handle registered with src ending in `/build/css/frontend-oauth.css`, non-empty version, `rtl` extra === `'replace'`; (c) missing `frontend-oauth.asset.php` (stub `is_readable` / `file_exists` to return false) → fallback version applied (plugin version constant), no PHP warning, handle still registered; (d) malformed `frontend-oauth.asset.php` (returns non-array or missing keys) → same silent-fallback path; (e) CLI query var truthy AND OAuth predicate false → `enqueue_styles()` registers ZERO handles (Phase 7 owns CLI). Use `#[DataProvider]` PHP attribute (B9), NOT `@dataProvider` docblock. Namespace `AcrossAI_MCP_Manager\Tests\Public`; use `WP_UnitTestCase`.
...
- [ ] T028 [P] **SEC-008-001 mitigation**: add a CI check under `.github/workflows/` (or extend an existing workflow) that runs `npm ci && npm run build && git diff --exit-code build/`. Exit non-zero if the committed `build/` tree drifted from a clean rebuild. Track under the release-infrastructure epic; not blocking Phase 8 merge but MUST land before `feature/issue-3 → main` cutover.
