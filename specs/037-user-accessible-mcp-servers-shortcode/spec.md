...
- **FR-029** *(added post-initial-implementation — screenshot feedback: "I do not see the how to connect steps")*: When the shortcode's `show_config="1"` attribute is set (default `1`), each DTO's `<li>` MUST additionally render a "how to connect" details block containing content sourced from shipped upstream helpers — never a bespoke re-implementation:
  - **`client` category** — MUST instantiate the `AbstractMCPClient` subclass named in the DTO's `meta.class` field, then call `get_config_file()`, `get_top_level_key()`, `get_config_snippet( $server_url, AbstractMCPClient::EMPTY_TOKEN_PLACEHOLDER )`, and `get_instructions()`. Render the returned config as a `<pre>` block (JSON via `wp_json_encode` when array-shaped; verbatim string otherwise), the config-file path in `<code>`, and the top-level key in `<code>`. `$server_url` MUST be computed by the abstract base and passed via the projection (see FR-030) — the concrete renderer MUST NOT re-derive it.
  - **`npm` category** — MUST substitute the two `%s` placeholders in the DTO's `meta.command_template` with `home_url()` and the server slug, respectively, then render the result as a `<pre>` block. This preserves NPM's contract as a single-source-of-truth template documented by F035.
  - **`ai_connector` category** — MUST render a short informational block explaining that AI Connectors use OAuth on the provider side (no local paste-in config exists). This is the intentional shape for connectors — no config snippet is needed at F038's layer.
  - Every category MUST render a "You need a WordPress Application Password" auth notice unless suppressed by `show_config="0"`.
  - Malformed DTOs (missing `meta.class`, non-existent class, class not extending `AbstractMCPClient`, missing `command_template` for NPM) MUST silent-drop the details block only — the DTO's icon + name still render, matching F038's defensive posture.

- **FR-030** *(added post-initial-implementation)*: `AbstractUserServersRenderer::get_accessible_servers()` MUST include a `server_url` field in each server projection entry, computed as `rest_url( trailingslashit( $namespace ) . $route )` where `$namespace` is the row's `server_route_namespace` (falling back to `'mcp'`) and `$route` is the row's `server_route` (falling back to `server_slug`). This matches how F013's `MCPClientsBlock` computes the same URL at `MCPClientsBlock.php:224-226` — F038 mirrors the F013 shape verbatim to avoid a divergent URL derivation.

- **FR-031** *(added post-initial-implementation)*: Two new shortcode attributes MUST exist alongside the FR-011 set: `show_config` (default `'1'`) toggles the FR-029 details block; `show_instructions` (default `'1'`) toggles the per-DTO instructions paragraph inside that block. Both attributes MUST accept the same `'1'` / `'0'` string convention as `show_description` (FR-011).
