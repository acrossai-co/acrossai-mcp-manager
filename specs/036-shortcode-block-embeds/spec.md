...
- **SC-006**: Grep gate on `includes/Embeds/`: zero import of `AcrossAI_MCP_Manager\Public\Discovery\ConnectionMethodRegistry` (only the `public/Renderers/EmbedBlock/` layer imports F035; `includes/Embeds/` is the pure-domain state layer and MUST NOT reach across into public).
