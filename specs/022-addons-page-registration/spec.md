...
- **FR-014**: `composer.json` MUST bump `acrossai-co/main-menu` from `0.0.14` to `0.0.17` so the vendor's `fs_menu` override API (0.0.16) and `MenuRegistrar::register()` no-op (0.0.17, letting Freemius's own `menu.addons` own the Add-ons row) reach production.
