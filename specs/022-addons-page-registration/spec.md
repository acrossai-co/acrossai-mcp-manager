...
- Q: For product 34418 specifically, which Freemius auto-submenus should surface? → A: **Five on, `addons` off** — `account`, `contact`, `support`, `upgrade`, `pricing` all `true`; `addons` `false` so Freemius does NOT add a second Add-ons submenu (the vendored `MenuRegistrar` already registers the canonical Add-ons row at slug `acrossai-addons` — enabling Freemius's own would produce a duplicate nav entry). The plugin's `fs_menu` block in `includes/Main.php` spells out every key explicitly so the visible policy lives at the call site — flip any boolean there to change operator UX without a vendor release. The vendor's `FreemiusInitializer::DEFAULT_MENU` is a fallback for consumer plugins that don't pass `fs_menu` at all — this plugin declares its own values so it never inherits from the fallback.
