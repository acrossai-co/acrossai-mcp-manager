...
- Q: For product 34418 specifically, which Freemius auto-submenus should surface? → A: **`account`, `contact`, `support`, `addons` all on; `upgrade` and `pricing` off**. The plugin's `fs_menu` block in `includes/Main.php` spells out every key explicitly so the visible policy lives at the call site — flip any boolean there to change operator UX without a vendor release. `upgrade` and `pricing` stay off because pricing/upgrade UX is served by the vendored Add-ons page. The vendor's `FreemiusInitializer::DEFAULT_MENU` is a fallback for consumer plugins that don't pass `fs_menu` at all — this plugin declares its own values so it never inherits from the fallback.
