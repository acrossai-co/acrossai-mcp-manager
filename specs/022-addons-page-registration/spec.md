...
- **FR-004**: The `AddonsPage` constructor MUST receive an `$args` array with the four keys: `fs_product_id = '31226'`, `fs_public_key = 'pk_4f369b07d1fc7cadbc272ce96d75e'`, `fs_slug = 'acrossai-mcp-manager'`, and `fs_menu` (associative array declaring the plugin's intent for each Freemius auto-submenu — see FR-016). Values are stored inline (no runtime lookup, no filter indirection).
