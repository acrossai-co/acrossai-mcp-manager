...
- Q: What Freemius credentials should the plugin pass to `\AcrossAI_Addon\AddonsPage`? → A: **User-provided real credentials**: `fs_product_id = 34413`, `fs_public_key = pk_1dd32ce8eb65f3bcfd0c9df459e7d`, `fs_slug = acrossai-mcp-manager`. Stored inline in `includes/Main.php` alongside the sibling plugin's precedent — no filter or option-based indirection.
...
- Q: For product 34413 specifically, which Freemius auto-submenus should surface? → A: **`account` + `contact` on; `support` off**. The `support` row (wp.org Support Forum link) is redundant with the shared AcrossAI support surface, so the plugin's inline `fs_menu` sets it to `false`. `upgrade` / `pricing` / `addons` also stay `false` (owned by the Add-ons page). This is the plugin-specific operator preference and lives in `includes/Main.php` — the vendor package's `DEFAULT_MENU` still has `support => true` so other consumer plugins are unaffected.
...
- **FR-004**: The `AddonsPage` constructor MUST receive an `$args` array with the four keys: `fs_product_id = '34413'`, `fs_public_key = 'pk_1dd32ce8eb65f3bcfd0c9df459e7d'`, `fs_slug = 'acrossai-mcp-manager'`, and `fs_menu` (associative array declaring the plugin's intent for each Freemius auto-submenu — see FR-016). Values are stored inline (no runtime lookup, no filter indirection).
...
- **NG-003**: This feature does NOT ship or register any Freemius add-on plugin definitions. The base plugin's Add-ons page will show either "No add-ons yet" or Freemius-provided add-ons depending on what has been published on the Freemius side for product id 34413. Populating the add-on catalog is out of scope.
