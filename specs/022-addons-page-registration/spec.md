...
- Q: For product 34418 specifically, which Freemius auto-submenus should surface? → A: **`account`, `contact`, `addons` on; `support`, `pricing`, `upgrade` off** — treating product 34418 (`acrossai-add-ons`) as the AcrossAI *umbrella*: one shared Freemius product for every AcrossAI plugin's Add-ons page and license-activation surface. `addons` is the whole point of the umbrella (single Add-ons page for the ecosystem); `account` is where every add-on license activates; `contact` is the shared contact surface. `support` is off because Freemius links it to a WP.org forum keyed on the product's slug — the umbrella product isn't on WP.org, so the link would be dead; per-plugin support belongs in each plugin's own main-menu pages. `pricing`/`upgrade` are off because pricing/upgrade belongs to individual add-ons, not the umbrella, and with `has_paid_plans => false` on the umbrella these would render Freemius' placeholder anyway. The plugin's `fs_menu` block in `includes/Main.php` spells out every key explicitly with a docblock explaining each choice.
...
- **FR-014**: `composer.json` MUST bump `acrossai-co/main-menu` from `0.0.14` to `0.0.18` so the vendor's `fs_menu` override API (0.0.16), `MenuRegistrar::register()` no-op (0.0.17), and `fs_has_addons` override (0.0.18 — required to actually surface the Freemius Add-ons row per Freemius SDK's `has_addons()` gate) all reach production.
