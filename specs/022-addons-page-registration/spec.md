...
- Q: What Freemius credentials should the plugin pass to `\AcrossAI_Addon\AddonsPage`? → A: **User-provided real credentials**: `fs_product_id = 31226`, `fs_public_key = pk_4f369b07d1fc7cadbc272ce96d75e`, `fs_slug = acrossai-mcp-manager`. Stored inline in `includes/Main.php` alongside the sibling plugin's precedent — no filter or option-based indirection.
- Q: Do we want Freemius's Account / Contact Us / wp.org Support Forum auto-submenus visible under the AcrossAI menu? → A: **Yes — all three.** Since the vendored `\AcrossAI_Addon\FreemiusInitializer` hardcoded all three to `false`, this feature also bumps `acrossai-co/main-menu` from `0.0.14` to `0.0.15` (upstream commit `a58dec9`) which flips the three defaults to `true` at the package level. Every consumer plugin of the package inherits the same behavior — the decision is a policy change of the shared package, not a per-plugin override. `upgrade`, `pricing`, and `addons` stay `false` (pricing/upgrade are handled by the Add-ons page itself; a second `addons` submenu would duplicate the vendor's own Add-ons registration).
...
- **FR-004**: The `AddonsPage` constructor MUST receive an `$args` array with exactly three keys: `fs_product_id = '31226'`, `fs_public_key = 'pk_4f369b07d1fc7cadbc272ce96d75e'`, `fs_slug = 'acrossai-mcp-manager'`. Values are stored inline (no runtime lookup, no filter indirection).
...
- **FR-014**: `composer.json` MUST bump `acrossai-co/main-menu` from `0.0.14` to `0.0.15` so the vendor's flipped-on `account` / `contact` / `support` menu defaults reach production.
...
- **NG-003**: This feature does NOT ship or register any Freemius add-on plugin definitions. The base plugin's Add-ons page will show either "No add-ons yet" or Freemius-provided add-ons depending on what has been published on the Freemius side for product id 31226. Populating the add-on catalog is out of scope.
